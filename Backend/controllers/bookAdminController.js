...
const handleAddBook = async (req, res) => {
    if(req.user.role == 1){
        ...
        const {title, author, publisher, price, language, isbn, totalpage, publishyear, edition, discount, category, description} = req.body;
        ...
        try {
            const formatTitle = title.toLowerCase().split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
            const formatAuthor = author.toLowerCase().split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
            const formatPublisher = publisher.toLowerCase().split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
            const categories = category.split(",").map((cat)=> new mongoose.Types.ObjectId(cat));
...
const handleDeleteBook = async (req, res) => {
    if(req.user.role == 1){
        ...
        if(!bookId || bookId == '') return res.status(400).json({"Result":false,"Data":"Book ID is required"});
...
const handleUpdateCategory = async (req, res) => {
    if(req.user.role == 1){
        ...
        if(!newName || newName == '') return res.status(400).json({"Result":false,"Data":"Category name is required"});

        let str = newName.trim();
        str = str.charAt(0).toUpperCase() + str.slice(1);

        const foundCategory = await Category.findOne({_id:catId}).exec();
        if(!foundCategory) return res.status(403).json({"Result":false, "Data":"Category doesm't exist"});
...
const handleDeleteCategory = async (req, res) => {
    if(req.user.role == 1){
        ...
        if(!catId || catId == '') return res.status(400).json({"Result":false,"Data":"Category ID is required"});
...
export {handleAddBook, handleDeleteBook, handleAddCategory, handleGetCategory, handleGetAllBooks, handleUpdateCategory, handleDeleteCategory};